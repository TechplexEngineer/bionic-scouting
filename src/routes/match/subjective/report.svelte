...
<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			...
			await Swal.fire({
				icon: 'error',
				title: 'Oops...',
				html: `Unable to pull data from google sheets<br>${e}`,
				showCloseButton: true
			});
		}
	});
	...
	const OnlyOurAlliance = (value: RxDocument<MatchSubjReport>, index, self) => {
		...
		return (
			(weAreBlue && blueTeams.includes(value.teamNumber)) ||
			(!weAreBlue && !blueTeams.includes(value.teamNumber))
		);
	};
...
<div class="content">
	<div class="d-flex">
		...
		<div class="flex-1 text-end">
			...
			<Select
				id="match"
				items={matchSelections}
				bind:value={selectedPrepMatch}
			...
			<tbody>
					...
					{#each ['red', 'blue'] as color}
						{#each selectedPrepMatch?.value.alliances[color].teamKeys as t}
							<td>
								...
								{Math.round(
									parseFloat(getFrom(scoutingDataAverages, t.replace('frc', ''), 'Avg Auto Hi')) +
										parseFloat(getFrom(scoutingDataAverages, t.replace('frc', ''), 'Avg Auto Lo')),
									2
								)}
								| {Math.round(
									parseFloat(getFrom(scoutingDataAverages, t.replace('frc', ''), 'Max Auto Hi')) +
										parseFloat(getFrom(scoutingDataAverages, t.replace('frc', ''), 'Max Auto Lo')),
									2
								)}
							</td>
						{/each}
				...
				<tr>
					...
					<td colspan="3">
						...
						opr: {Math.round(
							getSBData(statboticsData, selectedPrepMatch?.value.matchKey)?.opr_win_prob * 100,
							2
						)}
						% &mdash; mix: {Math.round(
							getSBData(statboticsData, selectedPrepMatch?.value.matchKey)?.mix_win_prob * 100,
							2
						)}
						%
					</td>
					...
					<td colspan="3">
						<!--elo:{getSBData(statboticsData, selectedPrepMatch?.value.matchKey)?.elo_win_prob} &mdash;-->
						opr: {Math.round(
							(1 - getSBData(statboticsData, selectedPrepMatch?.value.matchKey)?.opr_win_prob) *
								100,
							2
						)}
						% &mdash; mix: {Math.round(
							(1 - getSBData(statboticsData, selectedPrepMatch?.value.matchKey)?.mix_win_prob) *
								100,
							2
						)}
						%
				...
				<tr>
					...
					<td colspan="3">
						...
						{Math.round(
							getSBData(statboticsData, selectedPrepMatch?.value.matchKey)?.red_rp_1_prob * 100,
							2
						)}% &mdash; Rung
						{getSBData(statboticsData, selectedPrepMatch?.value.matchKey)?.red_rp_2}:
						{Math.round(
							getSBData(statboticsData, selectedPrepMatch?.value.matchKey)?.red_rp_2_prob * 100,
							2
						)}%
					</td>
					<td colspan="3">
						...
						{Math.round(
							getSBData(statboticsData, selectedPrepMatch?.value.matchKey)?.blue_rp_1_prob * 100,
							2
						)}% &mdash; Rung
						{getSBData(statboticsData, selectedPrepMatch?.value.matchKey)?.blue_rp_2}:
						{Math.round(
							getSBData(statboticsData, selectedPrepMatch?.value.matchKey)?.blue_rp_2_prob * 100,
							2
						)}%
					</td>
				</tr>
	...
	<Line
		data={calcDataForTeleGraph(
			handlerScoutingData,
			extractRedTeamsFromMatch(selectedPrepMatch?.value),
			extractBlueTeamsFromMatch(selectedPrepMatch?.value)
		)}
		options={teleOptions}
	/>
