...
<script lang="ts">
    ...
    async function assignScoutToCurrentMatch(event) {
        ...
        // Remove this match from other scouts
        let ss = await db.super_scouts.find().where({active: true}).exec()
        ss = ss.filter(scout => {
            return scout.assignedMatches.includes(matchKey)
        });
        for (let scout of ss) {
            await scout.atomicUpdate(data => {
                data.assignedMatches = data.assignedMatches.filter(m => m !== matchKey);
    ...
    const removeScoutsFromMatch = async (match: RxDocument<Match>) => {
        modalOpen = false;
        // Remove this match from other scouts
        let ss = await db.super_scouts.find().where({active: true}).exec()
        ss = ss.filter(scout => {
            return scout.assignedMatches.includes(match.matchKey)
        });
        for (let scout of ss) {
            await scout.atomicUpdate(data => {
                data.assignedMatches = data.assignedMatches.filter(m => m !== match.matchKey);
                // console.log("remove", data.name, "from", matchKey, data.assignedMatches);
                data.updatedAt = new Date().getTime();
                return data;
            });
                ...
                <td rowspan="2">
                    ...
                    {#if matchContainsOurTeam(m)}
                        {#if superScoutsLoaded && matchIsAssigned(m)}
                            <button class="btn btn-outline-success" on:click={openModal(m)}>Change</button>
                            <br>
...
<Modal isOpen={modalOpen} toggle={toggleModal}>
    ...
    <ModalBody>
        ...
        <button class="btn btn-warning float-end mt-2" on:click={removeScoutsFromMatch(currentMatch)}>Clear Scouts
        </button>
