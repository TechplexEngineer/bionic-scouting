<script lang="ts">
    ...
    async function build() {
        ...
        const eventKey = settingEvent.value;
        ...
        for (const m of matches) {
            ...
            for (const t of redTeams) {
                ...
                const mm = await db.match_metrics.findOne().where(query).exec();
                if (mm) {
                    await mm.atomicUpdate(doc => {
                        doc.scoutName = scoutGroups[groupCounter % numGroups][`scout${scoutCounter++}`].label;
                        return doc;
                    });
                } else {
                    await db.match_metrics.insert({
                        eventKey: m.eventKey,
                        matchKey: m.matchKey,
                        teamNumber: t,
            ...
            for (const t of blueTeams) {
                ...
                if (mm) {
                    await mm.atomicUpdate(doc => {
                        doc.scoutName = scoutGroups[blueGroupCounter % numGroups][`scout${scoutCounter++}`].label;
                        return doc;
                    });
                } else {
                    await db.match_metrics.insert({
